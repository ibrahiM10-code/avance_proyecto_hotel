...
def ingresar_pasajero():
    ...
    responsable = int(input("\t" * 6 + "Indique si es pasajero responsable [1. Responsable | 0. Comun]: "))
    if valida_pasajero(responsable) == False:
        os.system("cls")
        print("\t" * 6 + valida_pasajero(responsable))
        responsable = int(input("\t" * 6 + "Indique si es pasajero responsable [1. Responsable | 0. Comun]: "))
...
def menu_consultas():
    while True:
        ...
        if opcion == 1:
            ...
            cantidad_mostrar = int(input("\t" * 6 + "Ingrese la cantidad de pasajeros que desea mostrar: "))
            os.system("cls")
            consultar_parcial(cantidad=cantidad_mostrar)
        elif opcion == 3:
            id_pasajero_mostrar = int(input("\t" * 6  + "Ingrese el ID del pasajero que desea consultar: "))
            os.system("cls")
            consultar_particular(id_pasajero=id_pasajero_mostrar)
        elif opcion == 4:
            opcion_volver_salir = input("\t" * 6 + "Desea salir?: " + colored("Si","green") + "/" + colored("No","red") + ": ").lower()
...
def modificar_pasajero():
    nuevos_datos = []
    consultar_todo()
    id_modificar = int(input("\t" * 6 + "Ingrese el ID del pasajero a modificar: "))
    datos_actuales = mostrar_particular(id=id_modificar)

    nuevos_datos.append(datos_actuales[0])
    os.system("cls")
    print("\n")
    opc = input("\t" * 2 + f"¿Desea modificar el nombre del pasajero? Datos actuales: {datos_actuales[1]} [SI/NO]: ").lower()
    if opc == "si":
        nuevo_nombre = input("\t" * 2 + "Ingrese el nuevo nombre del pasajero: ")
        nuevos_datos.append(nuevo_nombre)
    else:
        nuevos_datos.append(datos_actuales[1])

    opc = input("\t" * 2 + f"¿Desea modificar el apellido del pasajero? Datos actuales: {datos_actuales[2]} [SI/NO]: ").lower()
    if opc == "si":
        nuevo_apellido = input("\t" * 2 + "Ingrese el nuevo apellido del pasajero: ")
        nuevos_datos.append(nuevo_apellido)
    else:
        nuevos_datos.append(datos_actuales[2])

    opc = input("\t" * 2 + f"¿Desea modificar la direccion del pasajero? Datos actuales: {datos_actuales[3]} [SI/NO]: ").lower()
    if opc == "si":
        nuevo_direccion = input("\t" * 2 + "Ingrese el nuevo direccion del pasajero: ")
        nuevos_datos.append(nuevo_direccion)
    else:
        nuevos_datos.append(datos_actuales[3])

    opc = input("\t" * 2 + f"¿Desea modificar el telefono del pasajero? Datos actuales: {datos_actuales[4]} [SI/NO]: ").lower()
    if opc == "si":
        nuevo_telefono = input("\t" * 2 + "Ingrese el nuevo telefono del pasajero: ")
        nuevos_datos.append(nuevo_telefono)
    else:
        nuevos_datos.append(datos_actuales[4])

    opc = input("\t" * 2 + f"¿Desea modificar el rut del pasajero? Datos actuales: {datos_actuales[5]} [SI/NO]: ").lower()
    if opc == "si":
        nuevo_rut = input("\t" * 2 + "Ingrese el nuevo rut del pasajero: ")
        while(valida_rut(nuevo_rut) == False):
            print("\t" * 2 + "Rut invalido...")
            time.sleep(2)
            os.system("cls")
            nuevo_rut = input("\t" * 2 + "Ingrese nuevamente el nuevo rut del pasajero: ")
        nuevos_datos.append(nuevo_nombre)
    else:
        nuevos_datos.append(datos_actuales[5])

    opc = input("\t" * 2 + f"¿Desea modificar el email del pasajero? Datos actuales: {datos_actuales[6]} [SI/NO]: ").lower()
    if opc == "si":
        nuevo_email = input("\t" * 6 + "Ingrese el nuevo email del pasajero: ")
        while(valida_email(nuevo_email) == False):
            print("\t" * 6 + "Email invalido...")
            time.sleep(2)
            os.system("cls")
            nuevo_email = input("\t" * 2 + "Ingrese nuevamente el nuevo email del pasajero: ")
        nuevos_datos.append(nuevo_email)
    else:
        nuevos_datos.append(datos_actuales[6])

    opc = input("\t" * 2 + f"¿Desea modificar el responsable del pasajero? Datos actuales: {datos_actuales[7]} [SI/NO]: ").lower()
    if opc == "si":
        nuevo_responsable = input("\t" * 2 + "Ingrese el nuevo responsable del pasajero: ")
...
def eliminar_pasajero():
    consultar_todo()
    id_eliminar = int(input("\t" * 6 + "Ingrese el ID del pasajero a eliminar: "))
...
def menu_pasajero():
    while True:
        ...
        if opcion == 1:
            ...
            opcion_salir = input("\t" * 6 + "Desea salir? "+ colored("Si","green") + "/" + colored("No","red")+ ": ").lower()
            if opcion_salir == "si":
                os.system("cls")
                break
        else:
